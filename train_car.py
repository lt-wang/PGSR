...
from utils.loss_utils import l1_loss, ssim, lncc, get_img_grad_weight,cos_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    ema_mask_entropy_for_log = 0.0
    normal_loss, geo_loss, ncc_loss = None, None, None
    prior_normal_loss = None
    loss_mask_entropy = None
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # 创建mask：使用alpha通道和viewpoint_cam.mask（如果有的话）
        image_mask = viewpoint_cam.mask.float() if viewpoint_cam.mask is not None else torch.ones_like(image)
        alpha_mask = render_pkg["alpha"][0] > 0
        
        # 如果有gt mask，结合使用
        if viewpoint_cam.mask is not None:
            gt_mask = viewpoint_cam.mask[0] > 0
            # 同时满足两个条件的像素
            mask = alpha_mask & gt_mask
        else:
            mask = alpha_mask

        # ssim_loss = (1.0 - ssim(image[:, mask], gt_image[:, mask]))
        ssim_loss = (1.0 - ssim(image, gt_image*image_mask))
        # 如果使用外观模型并且SSIM损失较小，使用外观补偿后的图像
        if 'app_image' in render_pkg and ssim_loss < 0.5:
            app_image = render_pkg['app_image']
            # 只在mask区域计算L1损失
            if mask.sum() > 0:
                Ll1 = l1_loss(app_image[:, mask], gt_image[:, mask])
            else:
                # 如果mask为空，使用全图像损失
        ...
        if iteration > 7000:
            ...
            normal = render_pkg["rendered_normal"] * image_mask  # 渲染的法线
            depth_normal = render_pkg["depth_normal"] * image_mask  # 从深度计算的法线
            ...
            loss += (normal_loss)
        # 法线先验损失
        if viewpoint_cam.normal is not None and iteration > 30000:
            lambda_normal_prior = opt.single_view_weight
            rend_normal = render_pkg["rendered_normal"] * image_mask   # 渲染的法线
            surf_normal = render_pkg["depth_normal"] * image_mask  # 从深度计算的法线
            prior_normal = viewpoint_cam.normal * (image_mask  ).detach()
            ...
            prior_normal_error = cos_loss(prior_normal, rend_normal) + \
                                cos_loss(prior_normal, surf_normal)
            prior_normal_loss =  lambda_normal_prior * prior_normal_error
            # loss += prior_normal_loss
        #物体mask熵损失
        if viewpoint_cam.mask is not None and opt.lambda_mask_entropy > 0:
            rendered_opacity = render_pkg["alpha"]
            image_mask = viewpoint_cam.mask.float()
            o = rendered_opacity.clamp(1e-6, 1 - 1e-6)
            mask_entropy_error = -(image_mask * torch.log(o) + (1-image_mask) * torch.log(1 - o)).mean()
            # tb_dict["loss_mask_entropy"] = loss_mask_entropy.item()
            loss_mask_entropy = opt.lambda_mask_entropy * mask_entropy_error
            loss = loss + loss_mask_entropy
        ...
        with torch.no_grad():
            ...
            ema_prior_normal_for_log = 0.4 * prior_normal_loss.item() if prior_normal_loss is not None else 0.0 + 0.6 * ema_prior_normal_for_log
            ema_mask_entropy_for_log = 0.4 * loss_mask_entropy.item() if loss_mask_entropy is not None else 0.0 + 0.6 * ema_mask_entropy_for_log
            if iteration % 10 == 0:
                loss_dict = {
                    "Loss": f"{ema_loss_for_log:.{5}f}",
                    "Single": f"{ema_single_view_for_log:.{5}f}",
                    "mask": f"{ema_mask_entropy_for_log:.{5}f}",
                    "normal": f"{prior_normal_loss.item():.{5}f}" if prior_normal_loss is not None else "0.00000",
